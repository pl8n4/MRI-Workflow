#!/usr/bin/env bash
# benchmark_sswarper.sh
# Loops over a set of thread counts and repeats,
# times run_sswarper.sh, and prints a summary table.
...
echo "Benchmarking SSwarper for sub-${SUBJ_LABEL}"
for threads in "${THREADS[@]}"; do
  for ((rep=1; rep<=REPS; rep++)); do
    echo -n "  [threads=$threads | run #$rep]  … "
    # Run and capture the time (stdout/stderr suppressed here)
    t=$( { time ./run_sswarper.sh "$SUBJ_LABEL" "$threads" >/dev/null 2>&1; } 2>&1 )
    echo "${t}s"
